...
const stackCategories = [
  ...
  },
  {
    ...
    items: [
      ...
      "Redis (cache e message broker para filas de mensageria)",
    ],
  },
  {
    ...
    items: [
      ...
      "Docker Compose (orquestração de 6 serviços com networking privado e volumes persistentes)",
      "Docker Swarm (orquestração de containers em produção na VPS)",
      "Dockerfile multi-stage build (separação de build e runtime)",
      "Nginx (gateway unificado, load balancing futuro)",
      "Traefik v2.11 (reverse proxy com SSL automático via Let's Encrypt, roteamento por subdomínio, config dinâmica via YAML)",
      "VPS Ubuntu Linux (servidor dedicado com administração SSH/SFTP)",
      "Portainer CE (gerenciamento visual de containers Docker em produção)",
      ...
      "Deploy automatizado via Node.js (ssh2 + ssh2-sftp-client para transferência e execução remota)",
  ...
  },
  {
    ...
    items: [
      ...
      "Multi-tenant Reverse Proxy (múltiplos domínios na mesma VPS via Traefik)",
    ],
  },
  {
    title: "IA & LangChain",
  ...
  },
  {
    ...
    items: [
      ...
      "WAHA — WhatsApp HTTP API (gateway WhatsApp Business, self-hosted na VPS com volumes persistentes)",
      "SMTP/Gmail Gateway (campanhas com autenticação segura e threading)",
      "Axios (cliente HTTP com interceptors para frontend)",
      "Hunter.io (validação de emails e deliverability score)",
    ],
  },
  {
